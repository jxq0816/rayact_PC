...
(function () {
    ...
    SWFReference.eventHandler = function (a, b) {
        SWFReference._instances[a]._eventHandler(b);
    };
    ...
    StreamUploader.isValidFile = function (a) {
        return "undefined" != typeof File && a instanceof File;
    };
    StreamUploader.canUpload = function () {
    ...
    StreamUploader.prototype = {
        ...
        retry: function () {
            ...
            2 > this.retriedTimes ? this.resumeUpload()
                : (this.timeoutHandler && clearTimeout(this.timeoutHandler), this.timeoutHandler = setTimeout(function () {
                g.resumeUpload()
            }, 1E4));
        },
    ...
    Main.prototype = {
        ...
        cancelOne: function (file_id, stopping) {
            ...
            this.get("onCancel") ? this.get("onCancel")(file_id) : this.onCancel(file_id);
        ...
        validateFile: function (uploader) {
            var name = uploader.get("name"), size = uploader.get("size"),
                ...
                filters = this.get("aFilters"), valid = !1, msg = "上传格式有误",
                info = {
                    id: uploader.get("id"),
            ...
            if (!bStreaming && size > 2147483648) {
                this.uploadError({status: 100, statusText: "Flash最大只能上传2G的文件!"});
                return !1;
            }
    ...
    window.Stream = window.Uploader = Main;
    /** window.Uploader是SWF组件的关键字(保留) */
})();