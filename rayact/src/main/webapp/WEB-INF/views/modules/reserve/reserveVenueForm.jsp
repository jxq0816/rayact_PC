...
<head>
    ...
    <link href="${ctxStatic}/map/m_style.css" rel="stylesheet" />
...
<ul class="nav nav-tabs">
    <li><a href="${ctx}/reserve/reserveVenue/">场馆列表</a></li>
    <li class="active"><a href="${ctx}/reserve/reserveVenue/form?id=${reserveVenue.id}">场馆<shiro:hasPermission
            name="reserve:reserveVenue:edit">${not empty reserveVenue.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission
            name="reserve:reserveVenue:edit">查看</shiro:lacksPermission></a></li>
</ul>
<br/>
<form:form id="inputForm" modelAttribute="reserveVenue" action="${ctx}/reserve/reserveVenue/save" method="post"
           class="form-horizontal">
    <form:hidden path="id"/>
    <sys:message content="${message}"/>
    <ul class="nav nav-tabs" id="myTab">
        <li class="active"><a href="#home">基本信息</a></li>
        <li><a href="#profile">场地服务</a></li>
        <li><a href="#messages">地图位置</a></li>
        <li><a href="#settings">图片</a></li>
    </ul>
    ...
    <div class="tab-content">
        <div class="tab-pane active" id="home">
            <table id="contentTable" class="table table-bordered">
                <tr>
                    ...
                    <td>是否启用:</td>
                    <td><form:checkboxes path="available" items="${fns:getDictList('yes_no')}" itemLabel="label"
                                         itemValue="value" htmlEscape="false" class=""/></td>
                </tr>
                <tr>
                    <td>负责人:</td>
                    <td><sys:treeselect id="primaryPerson" name="chargeUser.id" value="${reserveVenue.chargeUser.id}"
                                        labelName="chargeUser.name" labelValue="${reserveVenue.chargeUser.id}"
                                        title="用户" url="/sys/office/treeData?type=3" allowClear="true"
                                        notAllowSelectParent="true"/></td>
                    <td>联系电话:</td>
                    <td>
                        <form:input path="tel" htmlEscape="false" maxlength="20" cssClass="input-xlarge"/>
                    </td>
                </tr>
                <tr>
                    <td>营业开始时间：</td>
                    <td><input name="startTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
                               ...
                               onclick="WdatePicker({dateFmt:'HH:mm:ss',isShowClear:false});"/></td>
                    <td>营业结束时间:</td>
            ...
            <table class="table table-bordered">
                    ...
                    <td colspan="3">
                        <div>
                            <div class="alternate-bar">
                                <a href="javascript:;" data-action="expandItem" data-expand="invoice">发票</a><span>/</span>
                                <a href="javascript:;" data-action="expandItem" data-expand="parking">停车</a><span>/</span>
                                <a href="javascript:;" data-action="expandItem" data-expand="sale">场馆卖品</a><span>/</span>
                                <a href="javascript:;" data-action="expandItem" data-expand="transit">公交</a><span>/</span>
                                <a href="javascript:;" data-action="expandItem" data-expand="subway">地铁</a><span>/</span>
                            </div>
                        </div>
                    </td>
                </tr>
                <tr>
                    <td colspan="4">
                        <div class="row">
                            <!-- 提供发票 -->
                            <div class="span4" data-expand-target="invoice" style="<j:if test="${empty reserveVenue.venueMoreService.invoice}">display: none;</j:if>">
                                <label for="">发票</label>
                                <div class="input-group">
                                    <input id="invoice" type="text" name="moreServiceBean.invoice" value="提供发票">
                                </div>
                            </div>
                            <!-- 提供停车 -->
                            <div class="span4" data-expand-target="parking" style="<j:if test="${empty reserveVenue.venueMoreService.parking}">display: none;</j:if>">
                                <label for="">停车</label>
                                <div class="input-group">
                                    <input id="parking" type="text" name="moreServiceBean.parking" value="免费停车">
                                </div>
                            </div>
                            <!-- 场馆卖品 -->
                            <div class="span4" data-expand-target="sale" style="<j:if test="${empty reserveVenue.venueMoreService.sale}">display: none;</j:if>">
                                <label for="">场馆卖品</label>
                                <div class="control-group">
                                    <input id="sale" type="text" name="moreServiceBean.sale">
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <!-- 身高 -->
                            <div class="span4" data-expand-target="transit" style="<j:if test="${empty reserveVenue.venueMoreService.transit}">display: none</j:if>">
                                <label for="">公交</label>
                                <div class="input-group">
                                    <textarea id="transit" name="moreServiceBean.transit" rows="4" cols="20"></textarea>
                                </div>
                            </div>
                            <!-- 体重 -->
                            <div class="span4" data-expand-target="subway" style="<j:if test="${empty reserveVenue.venueMoreService.subway}">display: none;</j:if>">
                                <label for="">地铁</label>
                                <div class="input-group">
                                    <textarea id="subway" name="moreServiceBean.subway" rows="4" cols="20"></textarea>
                                </div>
                            </div>
                        </div>
                    </td>
        ...
        <div class="tab-pane" id="messages">
            <table class="table table-bordered">
                <tr>
                    <td>地址：</td>
                    <td><form:input id="map_keyword" path="address" htmlEscape="false" maxlength="200" class="input-xlarge "/>
                ...
                <tr>
                    ...
                    <td>
                        X轴:
                        <input name="addressX" id="map_x" onkeyup="this.value=this.value.replace(/[^0-9.]/g,'')" readonly="readonly" value="${reserveVenue.addressX}" class="com_info_text" style="float:none">
                        &nbsp;&nbsp;
                        Y轴:
                        <input name="addressY" id="map_y" onkeyup="this.value=this.value.replace(/[^0-9.]/g,'')" readonly="readonly" value="${reserveVenue.addressY}" class="com_info_text" style="float:none">
                        &nbsp;&nbsp;
        ...
        <div class="tab-pane" id="settings">
            <table class="table table-bordered">
                <tr>
                    <td colspan="4">
                        <mechanism:upload id="financeSchoolPic" fdKey="venuePic" name="attMains1" exts=".jpg"
                                          btnText="添加"
    ...
    <div class="form-actions">
        <shiro:hasPermission name="reserve:reserveVenue:edit"><input id="btnSubmit" class="btn btn-primary"
                                                                     type="submit"
