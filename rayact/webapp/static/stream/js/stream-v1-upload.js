var configimg,_timg,configfile,_tfile,configvideo,_tvideo;
var imgTag=0,fileTag=0,videoTag=0;
var imgLimit=[".gif",".jpeg",".jpg",".bmp",".png"];
var fileLimit=[".txt",".doc",".docx",".xls",".xlsx",".pdf",".zip",".rar"];
var videoLimit=[".wmv",".wm",".asf",".asx",".rm",".rmvb",".ra",".ram",".mpg",".mpeg",".mpe",".vob",".dat",".mov",".3gp",".mp4",".mp4v",".m4v",".mkv",".avi",".flv",".f4v",".mts"];
var imgMap=new HashMap();
var fileMap=new HashMap();
var videoMap=new HashMap();

//绑定上传事件
var initUpload=self.setInterval("addUpload()",50);
function addUpload(){
	var imgArea=angular.element("#i_select_files_img").length;
	var fileArea=angular.element("#i_select_files_file").length;
	var videoArea=angular.element("#i_select_files_video").length;
	if(imgArea>0&&imgTag==0){
		$(".mod-publish-attach").eq(0).attr("class","mod-publish-attach");
		configimg =createUpload('i_select_files_img',imgLimit);
		_timg = new Stream(configimg);
		$(".mod-publish-attach").eq(0).attr("class","mod-publish-attach ng-hide");
		imgTag=1;
	}
	if(fileArea>0&&fileTag==0){
		$(".mod-publish-attach").eq(1).attr("class","mod-publish-attach");
		configfile =createUpload('i_select_files_file',fileLimit);
		_tfile = new Stream(configfile);
		$(".mod-publish-attach").eq(1).attr("class","mod-publish-attach ng-hide");
		fileTag=1;
	}
	if(videoArea>0&&videoTag==0){
		$(".mod-publish-attach").eq(2).attr("class","mod-publish-attach");
		configvideo =createUpload('i_select_files_video',videoLimit);
		_tvideo = new Stream(configvideo);
		$(".mod-publish-attach").eq(2).attr("class","mod-publish-attach ng-hide");
		videoTag=1;
	}
	if(imgTag==1&&fileTag==1&&videoTag==1){
		window.clearInterval(initUpload);
	}
}
...
var removePerson = function(id){
    //点移除@提及的用户时执行此方法
	angular.element('#mentionPerson').find("li").each(function(){
	...
	if(mentionList.length==0){
		angular.element('#blogMentionTitle').css('border-bottom','');
		angular.element('#blogMentionTitleTagsBox').css('border-bottom','');
	}
};
//-----------------------------------------------------@提及-------------------------------------------------------------------
/* 发布消息表单 */
ispaceControllers.controller('formPublishCtrl',['$scope', '$element', '$resource', 'MicroBlog',function($scope, $element, $resource, MicroBlog){
	    ...
	    //切换类型事件
	    $scope.changePubType=function(type){
	    	$scope.pubType = type;
	    	//输入框必填验证
	    	var inputArea=angular.element("#inputMsg");
	    	if(type=="txt"){
	    		inputArea.attr("required","required");
	    	}else{
	    		inputArea.removeAttr("required");
	    	}
	    	if(type=="img"){
	    		if(null!=_tfile){
    				javascript:_tfile.cancel();
    			}
    			if(null!=_tvideo){
    				javascript:_tvideo.cancel();
    			}
	    		$scope.listFile=[];
	    		$scope.listVideo=[];
	    		fileMap=new HashMap();
	    ...
	    $scope.removeImg = function(id){
	    	...
	    	deleteAttMain(id);
	    	imgMap.remove(id);
	    	$scope.listImg = imgMap.values();
	    };
	
	    $scope.cancelUploadFile = function(){
	        //点取消上传文件按钮执行此方法
	    	$scope.uploadFileData = false;
	    	javascript:_tfile.cancel();
	    };
	    $scope.removeFile = function(id){
	    	...
	    	deleteAttMain(id);
	    	fileMap.remove(id);
	    	$scope.listFile = fileMap.values();
	    };
	    
	    $scope.cancelUploadVideo = function(){
	        //点取消上传文件按钮执行此方法
	    	$scope.uploadVideoData = false;
	    	javascript:_tvideo.cancel();
	    ...
	    $scope.removeVideo = function(id){
	    	...
	    	deleteAttMain(id);
	    	videoMap.remove(id);
	    	$scope.listVideo = videoMap.values();
	    };
	    ...
	    //-----------------------------------------------------@提及-------------------------------------------------------------------
	    angular.element('#addUserHomeDirective').autocomplete(ctx+'/search/user/finduserByKey', {
	    	formatMatch : function(item) {
	    		return item.userName + item.email + item.officeName;
	    	},
	    	formatItem : function(item) {
	    		var photo;
	    		if(item.photoUrl.indexOf("http")>-1){
	    			photo=item.photoUrl;
	    		}else{
	    			photo=ctx+"/"+item.photoUrl;
	    		}
	    		return '<img src="' + (photo) + '" alt="">' + item.userName + " (" + item.email + ") " + item.officeName;
	    	},
	    	parse : function(data) {
	    		var rows = [];
	    	...
	    	var user = {id:item.userId,name:item.userName,userName:item.loginName};
	    	angular.element('#addUserHomeDirective').val("");//将输入框内容置为空
	    	angular.element('#addUserHomeDirective').next(".error").remove();//将提示信息删除
	    	if(mentionList.contains(user)){
	    		angular.element('#addUserHomeDirective').after('<label class="error" >不能添加重复的用户!');//添加提示信息
	    	}else{
	    		mentionList.push(user);//加入数据
	    		angular.element('#addUserHomeDirective').next(".error").remove();
	    		//添加数据后则加上白线
	    		angular.element('#blogMentionTitle').css('border-bottom','1px solid #f9f9f9');
	    		angular.element('#blogMentionTitleTagsBox').css('border-bottom','1px solid #f9f9f9');
	    		angular.element('#mentionPerson').append("<li data-id='"+item.userId+"'><a onclick=removePerson('"+item.userId+"') class='tag' title='删除' href='javascript:void(0)'>"+item.userName+"&lt;"+item.loginName+"&gt;</a></li>");
	    	}
	    });
	    ...
	    $scope.submit = function(){
			...
			//if($scope.inputMsg){
				var attIds="";
		    	var blog=new Object();
		    	blog.mentionList = mentionList;
		    	var type=$scope.pubType;
		    	blog.type=type;//类型
		    	//blog.content=$scope.inputMsg;//内容
		    	blog.content=document.getElementsByName("inputMsg")[0].value;
		    	if("txt"==type){
		    		if(''==blog.content){
		    			alert("请输入内容");
		    			document.getElementById("submit").disabled=false;document.getElementById("submit").innerHTML="发布";
		    			return false;
		    		}
		    	...
		    	if("img"==type){
		    		...
		    		for(var i=0;i<$scope.listImg.length;i++){
		    			...
		    			attIds+=$scope.listImg[i].id;
		    		}
		    	}
				if("file"==type){
					if($scope.listFile.length<1){
		    			alert("请添加文件");
		    			document.getElementById("submit").disabled=false;document.getElementById("submit").innerHTML="发布";
		    			return false;
		    		}
					for(var i=0;i<$scope.listFile.length;i++){
		    			...
		    			attIds+=$scope.listFile[i].id;
		    		}
		    	}
				if("link"==type){
					blog.link=document.getElementById("pubLink").value;
				}
				if("video"==type){
					if($scope.listVideo.length<1){
		    			alert("请添加视频");
		    			document.getElementById("submit").disabled=false;document.getElementById("submit").innerHTML="发布";
		    			return false;
		    		}
					for(var i=0;i<$scope.listVideo.length;i++){
		    			if(i!=0){
		    				attIds+=",";
		    			}
		    			attIds+=$scope.listVideo[i].id;
		    		}
				}
	    ...
	    $scope.saveBlog=function(blog){
	    	/*MicroBlog.ourList({type: $scope.articlesType,pageNo:1,pageSize:1},function(data){
	    		alert(data);
	    		$scope.articles=data;
	    		$scope.loading=false;
	    		if(null!=data.length&&data.length==10){
					window.onscroll=function(){
				    	//文档实际高度-可视范围高度=滚动条能拖动的最大值
				    	if(getScrollHeight()-getClientHeight()-getScrollTop()<window.screen.height/10){
				    		window.onscroll=function(){};
				    		$scope.addScroll();
				    	}
				    };
	    	...
	    	angular.element(".panel-group").children().eq(0).hide('slow',function(){
	    		angular.element(".panel-group").children().eq(0).remove();
	    	});
	    	document.getElementById("submit").disabled="";
	    	document.getElementById("submit").innerHTML="发布";
    		return;*/
	    	MicroBlog.saveBlog({},blog,function(){
	    		$scope.changePubType("txt");
	    		$scope.getArticles();
	    		setTimeout('document.getElementById("submit").disabled=false;document.getElementById("submit").innerHTML="发布";',500);
	    ...
	    Main.prototype = {
	    		...
	    		renderUI : function(file_id) {
	    			//选择文件后界面效果
	    			if("img"==$scope.pubType){
	    				imgMap.put(file_id, {imgUrl: ctx+'/static/images/temp/pic.jpg',id:file_id,progress: '0'});
	    				$scope.listImg = imgMap.values();
    				}
    				if("file"==$scope.pubType){
    					
    				}
    				if("video"==$scope.pubType){
	    		...
	    		cancelUploadHandler : function(event, b) {
	    			...
	    			var value = this.getNode("stream-cancel", progressNode).id;
	    			if (typeof deleteAttMain != 'undefined' && deleteAttMain instanceof Function) {
	    				deleteAttMain(value);
	    			}
	    		...
	    		uploadProgress : function(a) {
	    			...
	    			var bytesLoaded = a.bytesLoaded,a = Math.min(99.99, a.percentLoaded);
	    			100 > a && (a = parseFloat(a).toFixed(2));
	    			var percent = (this.totalUploadedSize + bytesLoaded) * 10000 / this.totalFileSize / 100, percent = Math.min(99.99, percent);
	    			100 > percent && (percent = parseFloat(percent).toFixed(2));
	    			if($scope.pubType=="img"){
	    				imgMap.remove(id);
	    				imgMap.put(id, {imgUrl: ctx+'/static/images/temp/pic.jpg',id:id,progress: a});
	    				$scope.listImg = imgMap.values();
	    			}
	    			if($scope.pubType=="file"){
	    				$scope.uploadFileData = {
	    					progress: percent
	    				};
	    			}
	    			if($scope.pubType=="video"){
	    		...
	    		uploadComplete : function(a) {
	    			...
	    			/*if (!this.config.customered) {
	    				...
	    				this.getNode("stream-cancel", progressNode).innerHTML = "删除";
	    				this.getNode("stream-cancel", progressNode).id = a.attId;
	    			...
	    			if (!this.config.customered) {
	    				...
	    				if($scope.pubType=="img"){
	    					imgMap.remove(id);
	    					imgMap.put(a.attId, {imgUrl: ctx+'/micro/blog/image/'+a.attId,id:a.attId});
	    					$scope.listImg = imgMap.values();
	    				}
	    				if($scope.pubType=="file"){
	    					fileMap.put(a.attId, {id:a.attId,name:a.fileName,type:a.fileType});
	    					$scope.listFile=fileMap.values();
	    					$scope.uploadFileData=null;
	    				}
	    				if($scope.pubType=="video"){
	    					videoMap.put(a.attId,{id:a.attId,name:a.fileName});
	    					$scope.listVideo=videoMap.values();
	    					$scope.uploadVideoData=null;
	    				}
	    			}
