...
$(document).ready(function () {
    ...
    $("#area_parent_id").change(function () {
        $.postItems({
            url: front + '/f/user/register/getCityData', data: {extId: $("#area_parent_id").val()},
            error: function (l, m, k) {
                alert('失败');
            }, success: function (values) {
                $("#area_id option").remove();
                $.each(values, function (i, n) {
                    $("#area_id").append("<option value='" + n.id + "'>" + n.name + "</option>");
                });
                $('.select2-chosen:last').text($('#area_id option:first').text());
            }
        });
    });
});